...
class Stop(base.BaseAction):
    ...
    def stop(self):
        ...
        try:
            ...
            LOG.exception(
                "Unexpected error occurred while stopping instance %(uuid)s: "
                "%(error)s",
    ...
    def post_condition(self):
        ...
        try:
            ...
            if not instance:
                LOG.info(
                    "Instance %s not found during post-condition check",
                    self.instance_uuid
                )
                # Instance not found is OK - it might have been deleted
                return True

            current_state = instance.status
            LOG.debug(
                "Instance %s post-condition check: state=%s",
                self.instance_uuid, current_state
            )

            if current_state != 'stopped':
                if not nova.wait_for_instance_state(
                    instance, 'stopped', 8, 10
                ):
                    LOG.warning(
                        "Instance %s may not be fully stopped (state: %s)",
                        self.instance_uuid, current_state
                    )
                    return False
            return True
